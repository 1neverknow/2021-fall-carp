...
class MAENS:
    def __init__(self, info):
        ...
        self.nsize = [1, 2, 1]
        self.trap_cnt = 0
        self.local_search = self.lns_mut1
    ...
    def lns1(self, ind: Solution, coef, nsize):
        inst_tasks = self.tasks
    ...
    def lns_mut2(self, sx: Solution):
        ...
        for move in self.operations:
            ...
            while imp:
                ...
                # local search operator
                new = self.lns2(sls, coef, 1, move)
                if new.fitness < sls.fitness:
                    sls = new
                    imp = True
                # if count1 > 50 and sls.quality < prev_quality:
                #     break
            best_moves.append(sls)
        sls = min(best_moves, key=self.get_fitness)
        ...
        for move in self.operations:
            ...
            while imp:
                ...
                prev_quality = sls.quality
                # local search operator
                sls = self.lns2(sls, coef, 1, move)
                if sls.fitness < prev_fitness:
                    imp = True
                # if count1 > 50 and sls.quality < prev_quality:
                #     break
            best_moves.append(copy.deepcopy(sls))
        sls = min(best_moves, key=self.get_fitness)
    ...
    def lns2(self, sls: Solution, coef, nsize, move):
        ind = copy.deepcopy(sls)
        inst_tasks = self.tasks
        ind.calculate_fitness(coef)

        if nsize == 1:
            # traditional move
            next_move = move(ind, coef)
            # 将task_route转化为ind.task_seq
            seg_starts = np.ones(ind.loads[0] + 2, dtype=int)
            seg_starts[1:1+ind.loads[0]] = np.where(ind.task_seq[1:ind.task_seq[0]] == 0)[0] + 1
            orig_ptr = seg_starts[next_move.orig_seg] + next_move.orig_pos - 1
    ...
    def solve(self):
        ...
        prev_best = self.best_fsb_solution.quality
        ...
        if self.best_fsb_solution.quality == prev_best:
            self.trap_cnt += 1
            if self.trap_cnt == 50:
                self.local_search = self.lns_mut2
        else:
            self.trap_cnt = 0
            self.local_search = self.lns_mut1